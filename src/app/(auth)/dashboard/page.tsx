  ...
  return (
    <main className="h-[calc(100vh-73px)] overflow-y-auto bg-slate-50 p-6 pb-10 lg:p-8">
      ...
      <div className="mb-6 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm text-blue-700">
        Atualização automática ativa a cada 15 segundos.
        {lastUpdate ? (
          <span className="ml-1">
      ...
      {loading ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center text-sm text-slate-500 shadow-sm">
          Carregando dashboard da sua empresa...
        </div>
      ) : (
        <>
          <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <MetricCard
              title="Oportunidades"
              value={resumo.totalOportunidades}
              helper="Vendas em recuperação"
            />

            <MetricCard
              title="Taxa de recuperação"
              value={`${resumo.taxaRecuperacao}%`}
              helper={`${resumo.convertidos} convertido${
                resumo.convertidos === 1 ? "" : "s"
              } de ${resumo.totalOportunidades}`}
            />

            <MetricCard
              title="Aguardando retorno"
              value={resumo.clientesParaAcompanhar}
          ...
          <div className="mb-6 grid gap-6 xl:grid-cols-[1.35fr_1fr]">
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="space-y-6">
                <ProgressRow
                  label="PIX pendente"
                  value={resumo.pixPendentes}
                  total={resumo.totalOportunidades}
                  barClassName="bg-amber-500"
                />

                <ProgressRow
                  label="Checkout abandonado"
                  value={resumo.checkoutAbandonados}
                  total={resumo.totalOportunidades}
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <ResultadoCard
                  title="Convertidos"
                  value={resumo.convertidos}
                  helper={formatCurrency(resumo.valorConvertido)}
                  className="border-emerald-200 bg-emerald-50 text-emerald-700"
                />

                <ResultadoCard
                  title="Aguardando"
                  value={resumo.aguardandoResposta}
          ...
          <div className="grid gap-6 xl:grid-cols-[1.35fr_1fr]">
            <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
              ...
              {resumo.ultimasOportunidades.length === 0 ? (
                <div className="p-10 text-center text-sm text-slate-500">
                  Nenhuma oportunidade em aberto até o momento.
                </div>
              ) : (
                <div className="divide-y divide-slate-100">
                  {resumo.ultimasOportunidades.map((venda) => (
                    <div
                      key={venda.pedido_id}
              ...
              <div className="grid gap-3 border-b border-slate-100 p-5 sm:grid-cols-2">
                ...
                <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                  ...
                  <p className="mt-2 text-sm font-bold text-slate-950">
                    {resumo.ultimoWebhook
                      ? getEventLabel(resumo.ultimoWebhook.evento)
                      : "Nenhum evento"}
                  </p>
                  <p className="mt-1 text-xs text-slate-500">
                    {resumo.ultimoWebhook
                      ? formatDate(resumo.ultimoWebhook.created_at)
                      : "Sem dados"}
                  </p>
                </div>
              </div>
